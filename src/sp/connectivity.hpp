...
namespace sp{
	...
	public:
		...
		int _relabel(T *ilabels, int lsteps, int width, int height){
      ...
      return mcount.size(); // return the number of isolated segments
		}
					...
					for (int c = 0; c < count; c++){
						...
						for (int d = 0; d < 8; d++){
							...
							if (mlabels[yy*width + xx] == 0){ // if (xx,yy) have not been accessed
								...
								if (new_label == old_label){ // if the label of (xx,yy) is identical to the label of (mx[0], my[0])
									mx[count] = xx;
									my[count] = yy;
									count++;
						...
						} // end searching the neighboring pixels of (mx[c],my[c])

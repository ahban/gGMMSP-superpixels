...
void cvtmatp(sp::HatP &ou, uint8_t *im, int W, int H){
  ou.create(H, W);
  for (int x = 0; x < W; x++){
    for (int y = 0; y < H; y++){
...
void cvtmatl(double *ol, sp::HatL &il, int W, int H){
  for (int x = 0; x < W; x++){
    for (int y = 0; y < H; y++){
      ol[y+x*H] = il(y, x);
    }
...
// Basic useage
//[labels] = mx_gGMMSP(image)
//[labels] = mx_gGMMSP(image, v_x)
//[labels] = mx_gGMMSP(image, v_x, v_y)
// Professional usage
//[labels] = mx_gGMMSP(image, v_x, v_y, e_c)
//[labels] = mx_gGMMSP(image, v_x, v_y, e_c, T)
//[labels] = mx_gGMMSP(image, v_x, v_y, e_c, T, lambda)
//[labels] = mx_gGMMSP(image, v_x, v_y, e_c, T, lambda, e_s)
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
  ...
  if (nrhs < 1 || nrhs > 7){
    ...
    mexPrintf("       [labels] = mx_gGMMSP(image)\n");
    mexPrintf("       [labels] = mx_gGMMSP(image, v_x)\n");
    mexPrintf("       [labels] = mx_gGMMSP(image, v_x, v_y)\n\n");
    ...
    mexPrintf("       [labels] = mx_gGMMSP(image, v_x, v_y, e_c)\n");
    mexPrintf("       [labels] = mx_gGMMSP(image, v_x, v_y, e_c, T)\n");
    mexPrintf("       [labels] = mx_gGMMSP(image, v_x, v_y, e_c, T, lambda)\n");
    mexPrintf("       [labels] = mx_gGMMSP(image, v_x, v_y, e_c, T, lambda, e_s)\n\n");
  ...
  int best_gpu = su::gpu::find_best_gpu();
  if (best_gpu < 0){
    mexErrMsgTxt("There is no NVIDIA GPU with capability of 2.0 or greater.");
  }
  CUDART_CALL_CHECK(cudaSetDevice(best_gpu));
  cudaDeviceProp prop;
  cudaGetDeviceProperties(&prop, best_gpu);
  mexPrintf("Running on \n  %s\n\n", prop.name);
  ...
  // warm up  
  gisp.segmentation_S(imgo, v_x, v_y, 2, e_c, e_s, lamb);
  
  mexCallMATLAB(0, NULL, 0, NULL, "tic");
  gisp.segmentation_S(imgo, v_x, v_y, T, e_c, e_s, lamb);
  gisp.dlab.download(hlab);
  
  int nsp = mg.merge(gisp.oL.data, gisp.oL.steps, hlab.data, hlab.steps, gisp.oL.width, gisp.oL.height, (v_x*v_y) >> 2);
  mexCallMATLAB(0, NULL, 0, NULL, "toc");
  
  sp::Util::relabel(gisp.oL, 1);
  mexPrintf("  %d superpixels have been generated.\n\n", nsp); 
  
  plhs[0] = mxCreateDoubleMatrix(H, W, mxREAL);
  
  double *ml = (double*)mxGetData(plhs[0]);
  cvtmatl(ml, gisp.oL, W, H);
  return;
}